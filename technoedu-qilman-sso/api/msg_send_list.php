...
$qry = "SELECT COUNT(*) FROM user_note WHERE from_user_id = '".$_POST["user_id"]."'";
...
$qry = "SELECT b.name, b.part, n.* FROM user_note AS n, user_basic = b WHERE n.from_user_id = '".$_POST["user_id"]."' AND b.id = n.to_user_id ORDER BY n.reg_datetime DESC".$qry_limit;

$i = 0;
if ($result = $db->query($qry)) {
	while ($row = $result->fetch_assoc()) {
		$arrJsonResultMsg[$i]["to_id"]		= $row["to_user_id"];
		$arrJsonResultMsg[$i]["to_name"]		= $row["name"];
		$arrJsonResultMsg[$i]["to_part"]		= $row["part"];
