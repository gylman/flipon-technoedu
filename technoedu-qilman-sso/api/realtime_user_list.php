...
$qry = "SELECT DISTINCT r.id AS idx, r.* FROM room_list AS r, room_member AS rm WHERE (r.user_id = '".$_POST["user_id"]."' AND r.end_datetime >= now()) OR (rm.user_id = '".$_POST["user_id"]."' AND rm.room_id = r.id AND r.end_datetime >= now()) ORDER BY start_datetime ASC";
...
if ($result = $db->query($qry)) {
	while ($row = $result->fetch_assoc()) {
		...
		$arrJsonResultRoom[$i]["max_users"]		= $row["max_member"];
		$arrJsonResultRoom[$i]["title"]			= $row["title"];
		$arrJsonResultRoom[$i]["pw"]				= $row["pw"];
		$arrJsonResultRoom[$i]["display_layout"]	= $row["layout_type"];
...
$arrJsonResult["room"] = $arrJsonResultRoom;
$db->close();
