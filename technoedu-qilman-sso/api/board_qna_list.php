...
$qry = "SELECT COUNT(*) FROM board_qna".$qry_search;
...
$qry = "SELECT * FROM board_qna".$qry_search." ORDER BY reg_datetime DESC".$qry_limit;
...
if ($result = $db->query($qry)) {
	while ($row = $result->fetch_assoc()) {
		...
		$arrJsonResultBoard[$i]["user_name"]	= $row["user_name"];
		$arrJsonResultBoard[$i]["phone"]		= $row["phone"];
		$arrJsonResultBoard[$i]["email"]		= $row["email"];
