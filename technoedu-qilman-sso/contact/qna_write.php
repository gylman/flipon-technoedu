...
#write #email1, #write #email2 { width:150px; }
...
#email3 { height: 34px;line-height: 34px; border: 1px solid #d2d2d2; color: #747474; width:80px; }
</style>
...
$(document).ready(function() {
	...
	$("#write_btn").click(function(){
		...
		//핸드폰
		var phone_sum = '';
		if( $('#hp1').val().length >0 && $('#hp2').val().length >0 && $('#hp3').val().length >0) {
			phone_sum = $('#hp1').val()+"-"+ $('#hp2').val()+"-"+ $('#hp3').val();
		}
		
		//이메일
		var email_sum = '';
		if($('#email1').val().length > 0 && $('#email2').val().length > 0){
			email_sum = $('#email1').val() + '@' + $('#email2').val();
		}
		*/
		...
		$.post( g_apiUrlRoot+"board_qna_write.php", postJsonData, function( dataJson) {
	...
	<div id="sub_top">
		<ul>
			...
			<li class="r2"><a href="notice_list.php">공지사항</a><a href="qna_write.php" class="sub_select">문의사항</a><a href="faq_list.php">FAQ</a><span class="path">HOME > FaceLink 고객센터 > 문의사항</span></li>
		...
		<div id="write">
			<form>
				...
				<ul style="width: 740px;">
					<li style="text-align:center;height:40px;word-break : nowrap;display : inline-block; text-overflow : clip; overflow : hidden; white-space : nowrap;">핸드폰<span class="star">*</span></li>
