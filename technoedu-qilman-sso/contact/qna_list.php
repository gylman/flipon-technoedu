...
function setList(now_page, now_limit){
	...
	$.post( g_apiUrlRoot+"board_qna_list.php", postJsonData, function( dataJson) {
...
$(document).ready(function() {
	$("#common_room_list").on("click", "ul~ul[name=board]", function(){
		...
		localStorage.setItem("com.facelink.contack.qna_list.page_num",g_now_page)
		localStorage.setItem("com.facelink.contack.qna_list.search_val", g_search_val);
		
		location.href = "./qna_content.php?bid=" + $(this).children(".col1").data("bid");
	});
	...
	//기본 선택 설정
	g_now_page = localStorage.getItem("com.facelink.contack.qna_list.page_num");
	g_search_val = localStorage.getItem("com.facelink.contack.qna_list.search_val");
	if(g_now_page != null){
		localStorage.removeItem("com.facelink.contack.qna_list.page_num");
		localStorage.removeItem("com.facelink.contack.qna_list.search_val");
	}else{
		g_now_page = 0;
		g_search_val = '';
	}
