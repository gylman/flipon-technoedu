...
tmedia_session_jsep.prototype.o_audio_src_id = null;
tmedia_session_jsep.prototype.o_video_src_id = null;
...
tmedia_session_jsep.prototype.__set = function (o_param) {
    ...
    switch(o_param.s_key){
        ...
        case 'audio-src-id':
            {
                this.o_audio_src_id = o_param.o_value;
                return 0;
            }

        case 'video-src-id':
            {
                this.o_video_src_id = o_param.o_value;
                return 0;
            }
        case 'mute-audio':
        case 'mute-video':
    ...
    if (this.o_pc) {
        if (this.o_local_stream) {
            ...
            if(!this.b_cache_stream || (this.e_type == tmedia_type_e.SCREEN_SHARE)) { // only stop if caching is disabled or screenshare
                this.o_local_stream.getTracks()[0].stop(); // scchoi modify MediaStream -> MediaStreamTrack.stop
            }
...
tmedia_session_jsep01.prototype.__get_lo = function () {
	...
	//alert( "audio_id:" + this.o_audio_src_id );

	if ( this.o_audio_src_id != null ) {
		o_audio_constraints.optional = [ { sourceId: this.o_audio_src_id }];	
	}

	if ( this.o_video_src_id != null ) {
		o_video_constraints.optional = [ { sourceId: this.o_video_src_id }];	
	}
        ...
        if(this.e_type == tmedia_type_e.AUDIO && (this.b_cache_stream && __o_jsep_stream_audio)){
            ...
            navigator.nativeGetUserMedia(
                    { 
                        ...
                        audio: (this.e_type == tmedia_type_e.SCREEN_SHARE) ? false : !!(this.e_type.i_id & tmedia_type_e.AUDIO.i_id) ? o_audio_constraints : false, // modify by scchoi 
