<?
// sipjs를 통한 접속방법으로 수정함
// meeting_layout.php 에서 불필요한 부분을 삭제(meeting_layout_test.php에서 참조) // scchoi 20200405
...
<link rel="stylesheet" type="text/css" href="../css/meeting_layout.css?v=1">
...
if (g_rid.length <= 0) {
	...
	if (connection != null) {
		...
		connection.send('{"cmd":"exit_room", "roomidx": "<?=$room_fid?>", "user_id":"'+g_user_id+'", "user_name":"'+g_user_name+'"}');
	}
...
$(document).ready(function() {
	...
	function exitRoom()
	{
		if ( confirm("회의를 종료합니다. 방을 나가시겠습니까?") != 0 )
		{
			if (connection != null) {
				//alert('{"cmd":"enter_exit", "roomidx": "<?=$room_id?>", "user_id":"'+g_user_id+'", "user_name":"'+g_user_name+'"}');
				//connection.send('{"cmd":"exit_room", "roomidx": "<?=$room_id?>", "user_id":"'+g_user_id+'", "user_name":"'+g_user_name+'"}');
			...
			$.post( g_apiUrlRoot+"room_invite_user.php", {rid:g_rid, user_id:inviteUserID}, function(dataJson) {
				if (dataJson.rt_code == 0) {
					var postJsonData = {
						to_id : inviteUserID,
						from_id : g_user_id,
						msg : g_user_name+'님이 회의 방에 초대하셨습니다\nMy 페이지에서 확인해주세요'
					}
					$.post( g_apiUrlRoot+"msg_send.php", postJsonData, function(dataJson) {
						alert(inviteUserID+'님을 회의방에 초대했습니다');
					}, 'json');
...
function room_init() {
	$.post( g_apiUrlRoot+"room_info.php", {rid:g_rid}, function(dataJson) {
		...
		// get room uid
		$.post('../mcutest/mcucomm.php', {mcuip:'115.68.14.22', cmd: 'GetConfList'} )
		.done( 
			function ( data ) {
				val = eval('(' + data + ')');
				if ( val == null ) {
...
<script type='text/javascript' src="room_comm.js?v=10"></script>
...
?>
		...
		<div class="left_area">
			...
			<div id="movie_option">
				...
				<a href="#" id="btn_camswitch" class="option8"><img src="../images/switch_cam.png" style="opacity:80%;"></a>
...
connection.onopen = function () {
	...
	connection.send('{"cmd":"enter_room", "roomidx": "<?=$room_fid?>", "user_id":"'+g_user_id+'", "user_name":"'+g_user_name+'", "user_email": "'+g_user_email+'", "roomaddr":"'+g_did+'@'+g_outboundSipSvrUrl.substring(6)+'"}');
...
$(document).on('click', '.joiner_mute_audio', function() {
	...
	if (parseInt($(this).parents('ul').data('mute-audio'), 10) == 0) {
		$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'setAudioMute', uid: g_uid, partId: partId, flag: "true"} );
		$(this).parents('ul').data('mute-audio', '1');
	} else {
		$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'setAudioMute', uid: g_uid, partId: partId, flag: "false"} );
...
$(document).on('click', '.joiner_mute_video', function() {
	...
	if (parseInt($(this).parents('ul').data('mute-video'), 10) == 0) {
		$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'setVideoMute', uid: g_uid, partId: partId, flag: "true"} );
		$(this).parents('ul').data('mute-video', '1');
	} else {
		$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'setVideoMute', uid: g_uid, partId: partId, flag: "false"} );
...
$(document).on('click', '.joiner_exit', function() {
	...
	if ( confirm(joinerID+"님을 퇴장시키시겠습니까?") != 0 )
	{
		...
		connection.send('{"cmd":"exit_force", "roomidx": "<?=$room_fid?>", "user_id":"'+joinerID+'"}');
		...
		if ( partId ) 
		{
			$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'RemovePart', uid: g_uid, partId: partId} );
...
function setChangePos(num, val)
{
    console.log("setChangePos:"+ num + " = " + val);
 	$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'setMosaicSlot', uid: g_uid, num: num, id:val} );
}
...
function compTypeChange(compType)
{
	mcuCompType = compType;
 	$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'setCompositionType', uid: g_uid, compType: compType, size:mcuScreenSize, profileId:mcuProfileId} );
	changePosSelData();
}
...
$('#btn_sip_invite').click(function() {
	...
	if ( sipuri.length < 8 ) {
 	...
 	$.post('../mcutest/mcucomm.php', {
                    mcuip: '115.68.14.22',
cmd: 'InviteSIPAgent', uid: g_uid, dest: sipuri } );
	setTimeout( updateConfRoomUserList, 100); 
...
{
	var message = prompt("초대 메시지를 입력해 주세요.", "회의에 초대합니다.");
...
function sendDrawingLog()
{
	...
	var msg = '{"cmd":"draw_log", "roomidx": "<?=$room_fid?>", "color": "'+pancolor+'", "w":"'+strokeWidth+'", "x":'+posLogX+', "y":'+posLogY+'}';
	connection.send(msg);
...
function sendPointerInfo(cmd, ox, oy, x, y)
{
	...
	var msg = '{"cmd":"'+cmd+'", "roomidx": "<?=$room_fid?>", "ox": "'+ox+'", "oy":"'+oy+'", "x":'+x+', "y":'+y+'}';
	connection.send(msg);
	//console.log("send:"+msg);
}
...
function SendChangePage(page)
{
	...
	var msg = '{"cmd":"change_page", "roomidx": "<?=$room_fid?>", "page":"'+curPage+'"}';
...
function SendChangeDoc(idx)
{
	...
	var msg = '{"cmd":"change_doc", "roomidx": "<?=$room_fid?>", "docidx":"'+idx+'"}';
...
$('#chat_send').click( function (e) {
	...
	var msg = '{"cmd":"chat", "roomidx": "<?=$room_fid?>", "user_id": "'+g_user_id+'", "user_name":"'+g_user_name+'", "content":"'+ content +'"}';
	connection.send(msg);
...
<script src='mouseup.js?v=3'></script>
